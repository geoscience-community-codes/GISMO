...
% Uses Nobuhito Mori's func_despike_phasespace function, attached below.
% (Perhaps it would be better to save that to a separate file name?)
% Glenn Thompson, 2009
[m, n] = size(w);
for i = 1 : numel(w)
  y = get(w(i), 'data'); 
  [y, ip] = func_despike_phasespace( y );
  w(i) = set(w(i), 'data', y);
end
w = reshape(w, m, n);
