...
% 
% The idea is to support the old calls to catalog by translating them into calls to readCatalog
% 
% TO DO:
...
%		NEW: 
%			cobj = readCatalog('datascope', 'dbpath', '/Seis/catalogs/aeic/Total/Total', 'snum', datenum(2009,1,1), 'enum', datenum(2010,1,1), 'minmag', 4.0, 'region', [-170.0 -135.0 55.0 65.0]);
%
...
%		NEW: 
			cobj = readCatalog('datascope', 'dbpath', '/Seis/catalogs/aeic/Total/Total', 'subset_expression', 'time > "2009/1/1" & time < "2010/1/1"' & ml > 4 & lon > -170.0 & lon < -135.0 & lat > 55.0 & lat < 65.0');
...
if strcmp(dataformat, 'antelope')
	...
	if exists('p.Results.dbeval','var') 
		%cobj = readCatalog('datascope', 'dbpath', filepath, 'subset_expression', p.Results.dbeval, varargin{:})
	else
		%cobj = readCatalog('datascope', 'dbpath', filepath, vargarin{:})
	end
end
