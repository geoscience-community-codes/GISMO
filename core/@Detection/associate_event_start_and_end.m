...
%   catalogobj = detobj.associate(maxtimediff, sites);
    ...
    size(catalogobj.ontime)
    size(lastDetectionTime)
    catalogobj = Catalog(otime, olon, olat, [], [], {}, {}, 'ontime', catalogobj.ontime, 'offtime', lastDetectionTime);
    fprintf('%d OFF detections were determined to be duplicates using a time window of %.1f seconds\n',duplicatecount, maxTimeDiff);
