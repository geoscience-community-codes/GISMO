...
%                             ydata(isnan(ydata))=mindata; % we replace NaNs and Infs in data with nanmin(data) in ydata
                            if (obj(c).binsize == obj(c).stepsize) & ( strcmp(metric{cc}, 'counts') | strcmp(metric{cc}, 'energy') | strcmp(metric{cc}, 'cum_mag') )
                                ...
                                ylabel(ax(1), metric2label(metric{cc}, obj(c).binsize), 'Color', colors{1}, 'FontSize',12)
                                ...
                                ylabel(metric2label(metric{cc}, obj(c).binsize), 'FontSize',12)                        
                            end
                        end
%                         axis tight;
%                         a=axis;
                ...
                for c = 1 : numel(obj)
                    ...
                    for cc = 1: numsubplots % number of metrics to plot
                        ...
                        if numel(data)>0 & ~all(isinf(data)) & ~all(isnan(data))
                            ...
                            if (obj(c).binsize == obj(c).stepsize) & ( strcmp(metric{cc}, 'counts') | strcmp(metric{cc}, 'energy') | strcmp(metric{cc}, 'cum_mag') )
%                                 if strfind(metric{cc}, 'mag')
%                                     cumy = magnitude.eng2mag(cumsum(magnitude.mag2eng(y)));           
%                                     subplot(numsubplots,1,cc), [ax, h1, h2] = plotyy(obj(c).time, y, obj(c).time, cumy, @stairs, @plot );
%                                     set(h1, 'Color', colors{1});
%                                 else
                                    ...
                                    if numel(labels)==2
                                        ...
                                        ax(1) = subplot(numsubplots,1,cc)
                                        h1 = stairs(ax(1), t, y);
                                        %[ax, h1, h2] = plotyy(t, y, t, y, @stairs, @stairs );
                                    end
                                    set(h1, 'LineWidth', 3, 'Color', colors{1});
                                    
                                %end
                                datetick(ax(1), 'x','keeplimits');
                                ylabel(ax(1), labels{1}, 'Color', colors{1}, 'FontSize',12)
                                ylims = get(ax(1), 'YLim');
...
function labels = metric2label(metric, binsize)
    ...
    elseif strcmp(metric, 'mean_rate')
        labels{1} = sprintf('Mean # events per hour (binsize %s)', time_unit);
    elseif strcmp(metric, 'median_rate')
        labels{1} = sprintf('Median # events per hour (binsize %s)', time_unit);
    elseif strcmp(metric, 'cum_mag')
        labels{1} = sprintf('Cumulative Magnitude per hour (binsize %s)', time_unit);;
    elseif strcmp(metric, 'mean_mag')
        labels{1} = sprintf('Mean Magnitude per hour (binsize %s)', time_unit);
    elseif strcmp(metric, 'median_mag')
        labels{1} = sprintf('Median Magnitude per hour (binsize %s)', time_unit);
    end
end
