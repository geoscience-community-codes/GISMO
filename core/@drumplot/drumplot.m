...
classdef drumplot
    ...
    methods
        ...
        function obj = drumplot(varargin)
            ...
            %     'mpl' is 20, an attempt to set 'ytick' to 10 will result in
            %     'ytick' rounded to nearest multiple of 'mpl' (20 in this case).
            %     Note also that manually setting 'mpl' will result in an automatic
            %     adjustment of 'ytick'.
            %     DEFAULT = 30 (or auto if set to 0)
            %
            %  'catalog'-->(Catalog object from which event start/stop times
            %               are retrieved)
            %     EXAMPLE: h = drumplot(w,'catalog',cobj) where w is a 1x2 waveform
            %        object and cobj is a Catalog object
            %     DEFAULT = [] (No events)
            %
            %  'trace_color'-->(Trace Color)
            %     If wave contains only one waveform object, 'trace_color' can be
            ...
            %
            %   See also DRUMPLOT/PLOT
            %
            % Author: Dane Ketner, Alaska Volcano Observatory
            % Modified: Glenn Thompson 2016-04-19 to work with Catalog objects
            % Further modified: 2025-12 for CI-safe tests and stricter validation
            ...
            p.addParameter('mpl', obj.mpl, @(i) isnumeric(i) && isscalar(i) && isfinite(i) && floor(i) == i);
            ...
            p.addParameter('trace_color', obj.trace_color, ...
                @(x) isnumeric(x) && isvector(x) && numel(x)==3 && all(x>=0) && all(x<=1));
            ...
            p.addParameter('display', obj.display, @(s) ischar(s) || isstring(s));
            ...
            if ~isa(obj.wave,'waveform') || numel(obj.wave) ~= 1
                error('drumplot:InvalidWaveform', ...
                      'Input waveform must be a single waveform object');
            end

            if ~isempty(obj.catalog)
                if ~isa(obj.catalog,'Catalog') || numel(obj.catalog) ~= 1
                    error('drumplot:InvalidCatalog', ...
                          'Catalog must be a single Catalog object');
                end
            end

            if ~isempty(obj.detections)
                if ~isa(obj.detections,'Detection') || numel(obj.detections) ~= 1
                    error('drumplot:InvalidDetections', ...
                          'Detections must be a single Detection object');
                end
            end

            if ~isempty(obj.arrivals)
                if ~isa(obj.arrivals,'Arrival') || numel(obj.arrivals) ~= 1
                    error('drumplot:InvalidArrivals', ...
                          'Arrivals must be a single Arrival object');
                end
            end

            % Auto ytick handling: treat 0 as "choose something sensible"
            if obj.ytick == 0
                % Simple rule of thumb: at least one tick per line, but
                % no less than 30 minutes spacing for large mpl values.
                obj.ytick = max(obj.mpl, 30);
            end
        end
