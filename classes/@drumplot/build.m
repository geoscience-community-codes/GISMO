...
%BUILD: Drumplot builder function. A drumplot object (h) contains no
...
%  and a Catalog object to highlight over the drumplot. Calling build(h) will
%  assemble the drumplot figure based the information in h. Build returns
%  the figure handle if an output argument is provided, otherwise, there is
%  no output.
%
%USAGE: build(h) --> Create drumplot figure from drumplot object
%       fh = build(h) --> Also return drumplot figure handle
%
...
%  events from Catalog
    ...
    title(B.ax,'Building Drumplot...','FontSize',12)
    %refresh(B.fh)
    refresh(gcf)
    pause(0.1)
    ...
    elseif nargout == 1
       ...
       warning('DRUMPLOT:BUILD Too many outputs')
    end
end
