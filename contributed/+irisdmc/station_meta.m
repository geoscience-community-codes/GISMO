function [w,successList] = station_meta(w,varargin)
%station_meta adds station metadata to a waveform
% STATUS: LEGACY IRIS XML STATION SERVICE (pre-FDSN)
% Modern replacement should use FDSN StationXML via REST or ObsPy.
%
...
% CHECK ARGUMENTS
if numel(varargin)>1
   error('station_meta:incorrectNumberArguments','Incorrect number of arguments'); 
end
if ~strcmpi(class(w),'waveform')
    error('station_meta:missingWaveform','First input must be a waveform object');
end
checkEach = 0;
if numel(varargin)>=1
...
for n = 1:numel(indexList)
    ...
    %disp(httpLink);
    [s,success] = irisdmc.xml2struct(httpLink);
    if success
